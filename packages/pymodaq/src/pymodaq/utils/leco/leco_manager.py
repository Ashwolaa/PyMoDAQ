...
class LECONetworkMonitor:
    ...
    def __init__(
        ...
        self._components: dict[str, ComponentRecord] = {}  # name → record
    ...
    def refresh_components(self) -> list[ComponentRecord]:
        ...
        with self._lock:
            ...
            for name in current - known:
                rec = self._probe_component(name)
                self._components[name] = rec
                changed = True

            # Refresh last_seen for still-present components
            for name in current & known:
                self._components[name].last_seen = datetime.now()
                self._components[name].reachable = True
    ...
    def start_coordinator(
        ...
        """Launch a coordinator subprocess.
        ...
        -------
        CoordinatorRecord
            Record with ``alive=True`` once the process starts.
        """
    ...
    def stop_coordinator(self) -> None:
        """Terminate the coordinator process (if owned by this manager)."""
        rec = self._coordinator_record
        if rec is None or rec.process is None:
            return
    ...
    def add_proxy(
        ...
        ProxyRecord
        """
        cmd = [sys.executable, '-m', 'pyleco.utils.zmq_log_handler',
               '--in-port', str(in_port), '--out-port', str(out_port)]
        # pyleco proxy entry: use the proxy module
        cmd = [sys.executable, '-c',
               f'import zmq; ctx=zmq.Context(); f=ctx.socket(zmq.XSUB); f.bind("tcp://*:{in_port}");'
               f'b=ctx.socket(zmq.XPUB); b.bind("tcp://*:{out_port}"); zmq.proxy(f,b)']
    ...
    def _poll_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            # Fast tier: component list
            self.refresh_components()

            # Slow tier: actor details
            if t0 - last_detail_time >= self._detail_poll_interval:
                last_detail_time = t0
                with self._lock:
                    actor_names = [
                        n for n, r in self._components.items() if r.role == 'actor'
                    ]
